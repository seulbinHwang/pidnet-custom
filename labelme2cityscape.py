...
def deal_json(json_file):
    # json_file =  labelme/test_real.json
    data_cs = {}
    objects = []
    num = -1
    num = num + 1
    ...
    with open(json_file) as f:
        ...
        data = json.load(f)
        data_cs['imgHeight'] = data['imgHeight']
        data_cs['imgWidth'] = data['imgWidth']
...
def main():
    ...
    parser.add_argument('--json_input_dir', default='labelme', help='input annotated directory')
    parser.add_argument(
        '--output_dir', default='labelme', help='output dataset directory', )
    ...
    for json_name in os.listdir(args.json_input_dir):
        json_file = osp.join(args.json_input_dir, json_name)
        print("json_file:", json_file)
        data_cs = deal_json(json_file)
        if data_cs is None:
            continue
        json.dump(
            data_cs,
